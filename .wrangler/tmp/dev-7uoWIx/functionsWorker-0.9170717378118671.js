...
// .wrangler/tmp/pages-eaFOgx/functionsWorker-0.9170717378118671.mjs
...
async function onRequest(context) {
  ...
  __name(getListings, "getListings");
  __name2(getListings, "getListings");
  if (segments.length === 2 && segments[1] === "search") {
    try {
      ...
      const results = listings.filter(
        (business) => business.title.toLowerCase().includes(query) || business.categories?.some((cat) => cat.toLowerCase().includes(query)) || business.address?.toLowerCase().includes(query)
      );
  ...
  try {
    const listings = await getListings();
    const business = listings.find(
      (item) => item.id === businessId && (!category || item.categories?.includes(category))
    );
    if (!business) {
...
// .wrangler/tmp/bundle-MTs8FI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-MTs8FI/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9170717378118671.js.map
